...
import "./NotificationBell.css"; // تأكد من وجود ملف الستايل المشترك

export default function DoctorNotificationBell({ userId, onNavigate }) {
  const [notifications, setNotifications] = useState([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [showDropdown, setShowDropdown] = useState(false);
  const [loading, setLoading] = useState(false);

  // ✅ جلب إشعارات الطبيب فقط
  const fetchNotifications = async () => {
    if (!userId) return;
    setLoading(true);
    try {
      // نطلب الإشعارات بفلتر خاص بالطبيب
      const res = await fetch(`http://localhost:5000/notifications/${userId}?type=doctor`);
      if (res.ok) {
  ...
  useEffect(() => {
    fetchNotifications();
    // تحديث كل 15 ثانية لضمان وصول التنبيهات الحرجة بسرعة
  ...
  const handleNotificationClick = async (notification) => {
    ...
    // تحديث الحالة المحلية لإخفاء النقطة الحمراء
    ...
    // ✅ التوجيه الذكي حسب نوع الإشعار
    if (notification.data?.consultationId) {
      // إذا كان إشعار استشارة، نوجه الطبيب لقائمة المرضى أو الاستشارات
      if (onNavigate) onNavigate("doctor"); 
    } else if (notification.type === 'vital_update') {
      // إذا كان تحديث علامات حيوية، يمكن توجيهه لصفحة المريض مباشرة
      if (onNavigate) onNavigate("doctor");
    }
  };
